-- StrengthQuest - Multi-Skill Workout Support
-- Migration 007: Allow workouts with exercises from multiple skills
-- Created: 2026-02-23

-- ============================================================================
-- STEP 1: Make workouts.skill_id nullable
-- ============================================================================
-- Existing single-skill workouts keep their skill_id value.
-- New multi-skill workouts insert NULL.
ALTER TABLE workouts ALTER COLUMN skill_id DROP NOT NULL;

-- ============================================================================
-- STEP 2: Drop last_workout_at trigger
-- ============================================================================
-- The old trigger reads NEW.skill_id from workouts, which is now nullable.
-- We move last_workout_at updates into the RPC function instead.
DROP TRIGGER IF EXISTS trigger_update_last_workout_timestamp ON workouts;
DROP FUNCTION IF EXISTS update_last_workout_timestamp();

-- ============================================================================
-- STEP 3: Create multi-skill workout logging function
-- ============================================================================
...
DECLARE
  ...
  -- Each value: { set_count, primary_exercise_id, primary_name, primary_max_1rm, primary_max_weight, primary_max_reps }
...
BEGIN
  ...
  -- =========================================================================
  -- If rounding caused us to assign less than base_xp, add remainder to first skill
  IF v_xp_assigned < v_base_xp AND jsonb_array_length(v_skill_results) > 0 THEN
    ...
    BEGIN
      -- Find skill with most sets (first in array for simplicity)
      v_first_result := v_skill_results->0;
      v_first_skill_id := (v_first_result->>'skill_id')::INTEGER;

      -- Add remainder XP to that skill
      UPDATE user_skills
...
COMMENT ON FUNCTION log_multi_skill_workout IS 'Atomically logs a multi-skill workout. Inserts exercises/sets, detects PRs per skill, distributes XP proportionally by volume, checks level-ups, and updates tiers. Returns JSONB with per-skill results.';
