-- Migration 012: Track real PRs (max weight lifted) alongside estimated 1RM PRs
--
-- Adds independent "real PR" tracking: the heaviest weight ever lifted on a
-- primary exercise, regardless of rep count. This is separate from the estimated
-- 1RM PR (Epley formula). Both give +50 XP and can fire in the same workout.

-- ============================================================================
-- STEP 1: Schema changes — add max weight tracking to user_skills
-- ============================================================================
ALTER TABLE user_skills ADD COLUMN IF NOT EXISTS max_weight_lifted DECIMAL(6,2);
ALTER TABLE user_skills ADD COLUMN IF NOT EXISTS max_weight_lifted_at TIMESTAMPTZ;

-- ============================================================================
-- STEP 2: Backfill from existing workout data
-- ============================================================================
UPDATE user_skills us
SET max_weight_lifted = best.max_wt
FROM (
  SELECT w.character_id, e.skill_id, MAX(ws.weight_lbs) as max_wt
  ...
  GROUP BY w.character_id, e.skill_id
) best
WHERE us.character_id = best.character_id AND us.skill_id = best.skill_id;

-- ============================================================================
-- STEP 3: Updated log_multi_skill_workout with real PR detection
-- ============================================================================
...
DECLARE
  ...
  v_primary_max_weight_raw DECIMAL(6,2);
  v_old_pr DECIMAL(7,2);
  v_achieved_pr BOOLEAN;
  v_pr_exercise_name TEXT;

  -- Real PR variables
  v_old_max_weight DECIMAL(6,2);
  v_achieved_real_pr BOOLEAN;
  v_real_pr_bonus INTEGER;
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      v_set_weight := (v_set->>'weight')::DECIMAL(6,2);
      ...
      -- Track primary exercise for both estimated 1RM and raw weight
      IF v_exercise_id = (v_skill_entry->>'primary_exercise_id')::INTEGER THEN
        -- Track best estimated 1RM (for estimated PR detection)
        IF v_skill_entry->>'primary_max_1rm' IS NULL
          ...
          v_skill_entry := jsonb_set(v_skill_entry, '{primary_max_weight}', to_jsonb(v_set_weight));
          v_skill_entry := jsonb_set(v_skill_entry, '{primary_max_reps}', to_jsonb(v_set_reps));
        END IF;

        -- Track heaviest raw weight (for real PR detection)
        IF v_skill_entry->>'primary_max_weight_raw' IS NULL
           OR v_skill_entry->'primary_max_weight_raw' = 'null'::JSONB
           OR v_set_weight > (v_skill_entry->>'primary_max_weight_raw')::DECIMAL THEN
          v_skill_entry := jsonb_set(v_skill_entry, '{primary_max_weight_raw}', to_jsonb(v_set_weight));
        END IF;
      END IF;
  ...
  IF v_skill_keys IS NOT NULL THEN
    ...
    LOOP
      ...
      v_skill_pr_bonus := 0;
      v_achieved_pr := FALSE;
      v_achieved_real_pr := FALSE;
      v_real_pr_bonus := 0;
      v_pr_exercise_name := v_skill_entry->>'primary_name';
      v_tier_changed := FALSE;
      v_primary_max_weight := NULL;
      v_primary_max_reps := NULL;
      v_old_max_weight := NULL;

      SELECT current_pr_calculated_1rm, current_tier, current_level, current_xp, max_weight_lifted
      INTO v_old_pr, v_old_tier, v_old_level, v_old_xp, v_old_max_weight
      FROM user_skills
      WHERE character_id = p_character_id AND skill_id = v_skill_id;

      -- Estimated PR Detection (any rep count on primary exercise)
      ...
      -- Real PR Detection (heaviest weight on primary exercise)
      IF v_skill_entry->>'primary_max_weight_raw' IS NOT NULL
         AND v_skill_entry->'primary_max_weight_raw' != 'null'::JSONB THEN
        v_primary_max_weight_raw := (v_skill_entry->>'primary_max_weight_raw')::DECIMAL(6,2);

        IF v_old_max_weight IS NULL OR v_primary_max_weight_raw > v_old_max_weight THEN
          v_achieved_real_pr := TRUE;
          v_real_pr_bonus := 50;

          UPDATE user_skills
          SET max_weight_lifted = v_primary_max_weight_raw,
              max_weight_lifted_at = NOW()
          WHERE character_id = p_character_id AND skill_id = v_skill_id;
        END IF;
      END IF;

      -- Combined XP: base + estimated PR bonus + real PR bonus
      v_skill_xp := v_skill_base_xp + v_skill_pr_bonus + v_real_pr_bonus;
      ...
      -- Build per-skill result (includes both estimated and real PR data)
      v_skill_result := jsonb_build_object(
        ...
        'real_pr_bonus_xp', v_real_pr_bonus,
        'skill_xp', v_skill_xp,
        ...
        'achieved_real_pr', v_achieved_real_pr,
        'old_max_weight', v_old_max_weight,
        'new_max_weight', CASE WHEN v_achieved_real_pr THEN v_primary_max_weight_raw ELSE NULL END,
  ...
  SET total_xp = v_total_xp,
      ...
      achieved_pr = EXISTS (
        ...
        WHERE (r->>'achieved_pr')::BOOLEAN = TRUE
           OR (r->>'achieved_real_pr')::BOOLEAN = TRUE
      ),
